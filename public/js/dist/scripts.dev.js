...
fetch("./public/data/adatok.json").then(function (response) {
  ...
  function init() {
    var v_image; // navigációs nyilak

    $("#leftArrow").attr("src", data[0].leftArrow);
    $("#rightArrow").attr("src", data[0].rightArrow); // background beállítás, a háttér kép az adatok.json [0]-ban van eltárolva
    ...
    $("#foDiv").css("background-position", "center"); // indexképek felolvasása

    for (var i = 0; i < data.length; i++) {
      v_image = '<div id="div_' + i + '">' + '<img id="img_' + i + '" class = "indexImages" src =' + 'data:image/png;base64,' + data[i].img.data.$binary.base64 + '>' + '<span id="span_' + i + '" class="spanTooltip">' + data[i].name + '</span>' + '</div>';
  ...
  init(); // nagy kép töltése mindenhonnan, kattintás léptetés, első betöltés
  ...
  function fillbigImage(pNewPhoto, pNewImgId, pCurrentPhoto, pCurrentImgId) {
    ...
    $('#bigText').text(data[pNewPhoto].name); // megnő az index kép
  ...
  $(".indexImages").hover(function (handlerIn, handlerOut) {
    $(this).css("cursor", "pointer"); // a tooltip box miatt kell
